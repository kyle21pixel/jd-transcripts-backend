// Modern Clean JavaScript for JD Legal Transcripts
...
function initOrderForm() {
    ...
    function handleFileSelect(file) {
        ...
        if (!isValidType) {
            showStatus('Please select an audio or video file.', 'error');
            return;
        }
        ...
        // Update cost estimate based on file duration (if possible)
        if (file.type.startsWith('audio/') || file.type.startsWith('video/')) {
            estimateFileDuration(file);
        }
    }

    function estimateFileDuration(file) {
        // Create audio/video element to get duration
        ...
        element.addEventListener('loadedmetadata', () => {
            ...
            if (service && turnaround && duration > 0) {
                const basePrice = basePrices[service] || 1.25;
                const multiplier = turnaroundMultipliers[turnaround] || 1.00;
                const totalCost = (basePrice * multiplier * duration).toFixed(2);
                estimatedCostInput.value = `$${totalCost} (${duration} min estimated)`;
            }
    ...
    orderForm.addEventListener('submit', async (e) => {
        ...
        showStatus('Submitting your order...', 'loading');

        try {
            // Simulate API call (replace with actual endpoint)
            await simulateOrderSubmission(formData);
            
            showStatus('Order submitted successfully! You will receive a confirmation email shortly.', 'success');
            orderForm.reset();
            ...
            showStatus('Failed to submit order. Please try again or contact support.', 'error');
        } finally {
    ...
    function validateOrderForm() {
        ...
        requiredFields.forEach(fieldName => {
            ...
            if (!field.value.trim()) {
                showStatus(`Please fill in the ${fieldName} field.`, 'error');
                field.focus();
                isValid = false;
                return false;
            }
...
function removeFile() {
    ...
    // Recalculate estimate without file duration
    const serviceSelect = document.getElementById('service');
    const turnaroundSelect = document.getElementById('turnaround');
