...
function storeOrderLocally(orderData) {
    ...
    .catch(error => {
        ...
        const orders = JSON.parse(localStorage.getItem('jd-orders') || '[]');
        orders.push(orderData);
        localStorage.setItem('jd-orders', JSON.stringify(orders));
    });
}
...
function handleMpesaPayment(orderData) {
    ...
    const paymentData = {
        ...
        amount: Math.round(parseFloat(orderData.totalPrice) * 130), // Convert to KES
...
function pollPaymentStatus(checkoutRequestId, orderData, modal, styleSheet) {
    ...
    const pollInterval = setInterval(() => {
        ...
        fetch(`${serverUrl}/api/mpesa/status`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
